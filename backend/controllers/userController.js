...
exports.forgotPassword = catchAssyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    // if these two Stays valid in the database but never reaches the user (because the email failed to send),

//Then someone else who gets hold of that token (or tries to guess it) could potentially reset the password without authorization.
...
exports.updateProfile = catchAssyncErrors(async (req, res, next) => {
 ...
 // Get all users(admin)
exports.getAllUser = catchAssyncErrors(async (req, res, next) => {
